...
elif not Path.cwd().resolve().is_relative_to((home/'repository'/'.worktrees').resolve()):
...
with terminal_input() as packets:
    ...
    for packet in packets:
        ...
        for raw in packet.splitlines():
            ...
            verify_path = name+'-verify.py'
            Path(verify_path).write_text('from pathlib import Path\nassert Path('+repr(name+'.txt')+').read_text().strip() == '+repr(name)+'\n')
            subprocess.run(['git','add',name+'.txt',report_path,verify_path],check=True)
            ...
            report={'head':head,'summary':'Created '+name+' report','checks':[{'criterion':task['criteria'][0],'command':f'python3 {verify_path}'}],'assets':[{'path':report_path,'sha256':asset_hash}]}
            ...
            except Exception as e:print('Report failed:',str(e),e.read().decode() if hasattr(e,'read') else '',flush=True)
