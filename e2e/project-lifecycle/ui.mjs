...
const retired=async(project,worker)=>wait(()=>projectExecutors(project).length===0&&(!worker||!fs.existsSync(path.join(config.repo,'.worktrees',worker))),'verified executors did not retire for '+project);
...
const reviewAndRetire=async(requireHeld=false)=>{
  ...
  if(held){
    if(usesWorktree) assert.ok(fs.existsSync(path.join(config.repo,'.worktrees',worker)),'review-held worktree must remain available');
    else assert.ok(!fs.existsSync(path.join(config.repo,'.worktrees',worker)),'shared-checkout project must not create a worktree');
  ...
  if(worker) assert.ok(!fs.existsSync(path.join(config.repo,'.worktrees',worker)),usesWorktree?'accepted executor worktree must be deleted':'shared-checkout executor must not materialize a worktree');
};
