...
function harness(queue,replies,transport=null){
 ...
 const ctx=vm.createContext({peekSession:'test-worker',_peekKickFast:()=>peekNudges.push(Date.now()),Date,Set,console,Response,JSON,navigator:{onLine:true},_upqList:async()=>[],_uploadSyncPending:false,_syncChecklist:[],_syncBannerRequested:false,_syncPillText:'',_syncChecklistAt:0,updateConnectionStatus(){},_clearSyncTransientToast(){},_syncBannerBeacon:(phase,items)=>beacons.push({phase,n:(items||[]).length}),encodeURIComponent,decodeURIComponent,document:{getElementById:()=>element},drafts:[],offlineQueue:queue,_outboxActive:new Set(),describeOp:()=> 'test send',esc:s=>s,
