...
test('LC-COMPLEX-VERIFIED: selected-provider peers decompose linked work, adapt to changed gates, and independently verify every deliverable', async ({page,request},info) => {
  test.setTimeout(3_900_000);
  page.setDefaultTimeout(30_000);
  expect(process.env.AMUX_LIFECYCLE_LAB_ACK).toBe('dedicated-test-instance');
  const run = process.env.AMUX_LIFECYCLE_COMPLEX_RUN || `${lifecyclePrefix}complex-${Date.now()}`;
