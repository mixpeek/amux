...
test('LC-BROWSER-BACKGROUND: real browser does not focus the desktop and expires with saved profile intact',async({page,request},info)=>{
 ...
 try {
  ...
  await expect.poll(async()=>((await status()).browsers||[]).length,{timeout:40_000,intervals:[500,1000]}).toBe(0);owned=false;
  expect(await readFile(marker,'utf8')).toBe('Saved profile must survive process expiry');
  await info.attach('background-browser-proof',{body:JSON.stringify({profile:name,pid:started.pid,headless:started.headless,foregroundBefore:before,foregroundAfter:front(),foregroundSamples:foreground,ttl:Number(process.env.AMUX_LIFECYCLE_BROWSER_TTL_S),idleTTL:0,activityTTL:0,profileSurvived:true}),contentType:'application/json'});
