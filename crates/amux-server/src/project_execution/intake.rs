    ...
    fn project_reverify_completed_fanout_uses_durable_owner_after_executor_retirement() {
        ...
        state.store.write(move|c| {
            for row in &completed {
                ...
                c.execute("UPDATE issues SET status='verified',session=?2,evidence='retained verified evidence',execution_state=?3 WHERE id=?1",params![row.id,worker,serde_json::to_string(&super::super::planner::Execution{stage:if worker.is_some(){"verified"}else{""}.into(),worker:worker.unwrap_or("").into(),generation:1,input_hash:super::super::planner::input_hash(row),..Default::default()}).unwrap()])?;
            }
            Ok(WriteOutcome{applied:true,events:vec![]})
        }).unwrap();
