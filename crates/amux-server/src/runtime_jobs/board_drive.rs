...
async fn with_project_ticks<L, P, F>(legacy: L, mut project: F, cadence: std::time::Duration) -> L::Output
where
    L: std::future::Future,
...
pub fn spawn(state: AppState) -> super::PeriodicTask {
    ...
    super::spawn_periodic(JOB, secs, move || {
        ...
        async move {
            ...
            }, || crate::project_execution::driver::tick(&state), std::time::Duration::from_secs(secs.max(1))).await;
        }
    })
}
...
mod project_cadence_tests {
    ...
    fn slow_failing_legacy_sweep_preserves_project_report_progress_and_pause() {
        ...
        let ticks=Arc::new(AtomicUsize::new(0));let starts=Arc::new(AtomicUsize::new(0));
        let finished=Arc::new(tokio::sync::Notify::new());
        let legacy={let finished=finished.clone();let starts=starts.clone();async move {starts.fetch_add(1,Ordering::SeqCst);finished.notified().await;Err::<(),_>("legacy start failed")}};
        let result=tokio::time::timeout(Duration::from_secs(2),with_project_ticks(legacy,||{
            let db=db.clone();let ticks=ticks.clone();let finished=finished.clone();
            async move {
                let n=ticks.fetch_add(1,Ordering::SeqCst);
                db.write(move|c| {
                    if n==0 {assert!(planner::claim(c,"cadence","A").unwrap().applied);}
                    else if n==1 {
                        ...
                        let mut p=store::get(c,"cadence").unwrap().unwrap();p.policy.paused=true;
                        store::save(c,"cadence",p.revision,&p.policy,"test").map_err(store::sql_error)?;
                        assert!(!planner::claim(c,"cadence","B").unwrap().applied);
                        let p=store::get(c,"cadence").unwrap().unwrap();
                        assert!(planner::plan(c,&p).unwrap().iter().all(|p|p.waiting_reason.as_deref()==Some("project_paused")));
                        finished.notify_one();
                    }
                    Ok(crate::db::WriteOutcome{applied:true,events:vec![]})
                }).unwrap();
            }
        },Duration::from_millis(10))).await.expect("project progress must not wait for legacy completion");
        assert_eq!(result,Err("legacy start failed"));assert_eq!(starts.load(Ordering::SeqCst),1);assert_eq!(ticks.load(Ordering::SeqCst),3);
        });
    }
    ...
    async fn project_cadence_cancels_inflight_legacy_and_serial_negative_control_stalls() {
        ...
        let dropped=Arc::new(AtomicUsize::new(0));let ticks=Arc::new(AtomicUsize::new(0));
        let guard=Dropped(dropped.clone());
        let legacy=async move {let _guard=guard;std::future::pending::<()>().await};
        assert!(tokio::time::timeout(Duration::from_millis(65),with_project_ticks(legacy,||{let ticks=ticks.clone();async move{ticks.fetch_add(1,Ordering::SeqCst);}},Duration::from_millis(10))).await.is_err());
        assert_eq!(dropped.load(Ordering::SeqCst),1,"cancel drops the owned legacy operation; no detached sweep");
        assert!(ticks.load(Ordering::SeqCst)>1);
        let before=ticks.load(Ordering::SeqCst);tokio::task::yield_now().await;assert_eq!(ticks.load(Ordering::SeqCst),before);
