    ...
    for task in &d.tasks {
        ...
        if project.is_some()
            ...
            && (original_hash != crate::project_execution::planner::input_hash(&row)
                || task.action == "verify")
            && !crate::project_execution::outputs::authorization_hold(conn, &row)
                .map_err(crate::project_execution::store::sql_error)?
        {
