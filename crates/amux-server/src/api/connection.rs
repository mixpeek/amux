...
pub fn routes(dir: PathBuf) -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            post(sign_in).layer(axum::extract::DefaultBodyLimit::max(4096)),
        )
        .route(
            "/api/connection/certificate",
            post(certificate).layer(axum::extract::DefaultBodyLimit::max(100_000)),
        )
...
fn same_origin(headers: &HeaderMap, uri: &Uri) -> bool {
    ...
    let (Some(authority), Some(origin)) = (authority, origin) else {
        return false;
    };
    let Ok(expected) = reqwest::Url::parse(&format!("https://{authority}")) else {
        return false;
    };
    if origin != expected.origin().ascii_serialization() {
        return false;
    }
    if headers
        .get("sec-fetch-site")
...
pub async fn sign_in(
    ...
    headers: HeaderMap,
    input: Result<Json<SignIn>, axum::extract::rejection::JsonRejection>,
) -> Response {
    if !same_origin(&headers, &uri) {
        return refusal(StatusCode::FORBIDDEN, "same_origin_required");
    }
    let Ok(Json(input)) = input else {
        return refusal(StatusCode::BAD_REQUEST, "invalid_sign_in_request");
    };
    // This is intentionally independent of middleware's localhost admission.
    if !state.auth_token.as_deref().is_some_and(|expected| {
    ...
    let mut response = (
        [("cache-control", "no-store")],
        Json(json!({"ok":true,"reload":"/api/_clear_sw"})),
    )
        .into_response();
...
async fn status(
    State(state): State<AppState>,
    Extension(dir): Extension<TlsDirectory>,
    headers: HeaderMap,
) -> Response {
    let auth = if owner(&state, &headers) {
        "owner"
    } else if super::org::is_verified_local_member(&headers) {
        "member"
...
async fn certificate(
    ...
    OriginalUri(uri): OriginalUri,
    headers: HeaderMap,
    input: Result<Json<CertificateUpload>, axum::extract::rejection::JsonRejection>,
) -> Response {
    if !same_origin(&headers, &uri) {
        return refusal(StatusCode::FORBIDDEN, "same_origin_required");
    }
    if !owner(&state, &headers) {
        return refusal(StatusCode::FORBIDDEN, "owner_session_or_token_required");
    }
    let Ok(Json(input)) = input else {
        return refusal(StatusCode::BAD_REQUEST, "invalid_certificate_request");
    };
    let origin = headers
        .get("origin")
        .and_then(|h| h.to_str().ok())
    ...
    let result = tokio::task::spawn_blocking(move || {
        crate::tls::install_connection_certificate(
            &dir,
    ...
    match result {
        Ok(Ok(metadata)) => (
            [("cache-control", "no-store")],
            Json(json!({"ok":true,"saved":metadata,"applied":false,"restart_required":true})),
        )
            .into_response(),
        Ok(Err(err)) => {
...
mod tests {
    ...
    fn app(token: Option<&str>, dir: &std::path::Path) -> Router {
        ...
        routes(dir.join("tls"))
            .route(
                "/api/_clear_sw",
                get(super::super::static_files::clear_sw_landing),
            )
            .layer(axum::middleware::from_fn_with_state(
                state.clone(),
                super::super::org::local_member_identity,
            ))
            .with_state(state)
    }
    async fn call(
    ...
    async fn connection_security_sign_in_requires_explicit_valid_same_origin_token_even_on_loopback(
        ...
        let router = app(Some("owner-secret"), dir.path());
        for (path, body, origin, expected) in [
            (
                "/api/connection/session",
                json!({"token":""}),
                Some("https://localhost:18972"),
                401,
            ),
            (
                "/api/connection/session",
            ...
            (
                "/api/connection/session",
                json!({"token":"owner-secret"}),
                Some("http://localhost:18972"),
                403,
            ),
            (
                "/api/connection/session?_token=owner-secret",
                json!({"token":"owner-secret"}),
                Some("https://localhost:18972"),
                403,
            ),
        ...
        let (status, headers, body) = call(
            &router,
            "/api/connection/session",
            Some(json!({"token":"owner-secret"})),
            Some("https://localhost:18972"),
            Some("amux_member=revoked-member"),
        )
        .await;
        ...
        assert_eq!(revoked["owner_session"], "invalid");
    ...
    async fn connection_security_certificate_route_rejects_members_and_bad_origin_then_persists_matching_pair(
        ...
        let (_, _, member) = call(
            ...
            None,
            None,
            Some("amux_member=member"),
        )
        .await;
        assert_eq!(member["auth"], "member");
        assert_eq!(member["can_configure"], false);
        assert_eq!(
            call(
                &router,
                "/api/connection/session",
        ...
        for cookie in [
            None,
            Some("amux_member=member"),
            Some("__Host-amux_owner=revoked"),
        ] {
            ...
            assert_eq!(body["error"], "owner_session_or_token_required");
        ...
        assert_eq!(body["error"], "certificate_invalid_or_key_mismatch");
        assert_eq!(
            crate::tls::load_or_generate(&tls_dir).unwrap().cert_pem,
            original.cert_pem
        );
        ...
        assert_eq!(
            snapshot,
            std::fs::read(tls_dir.join("connection.pem")).unwrap()
        );
    }
}
