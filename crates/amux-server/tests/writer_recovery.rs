...
fn outcome(applied: bool) -> WriteOutcome {
    WriteOutcome {
        applied,
        events: vec![],
    }
...
fn store() -> (tempfile::TempDir, Arc<Store>) {
    ...
    store
        .write(|c| {
            c.execute_batch("CREATE TABLE recovery_marker (id INTEGER PRIMARY KEY)")?;
            Ok(outcome(false))
        })
        .unwrap();
    (dir, store)
...
fn count(store: &Store) -> i64 {
    store
        .read()
        .unwrap()
        .query_row("SELECT COUNT(*) FROM recovery_marker", [], |r| r.get(0))
        .unwrap()
}
...
fn mutation_panic_rolls_back_and_the_next_write_succeeds() {
    ...
    assert_eq!(
        count(&store),
        0,
        "panic must roll back the partial mutation"
    );
    store
        .write(|c| {
            c.execute("INSERT INTO recovery_marker VALUES (2)", [])?;
            Ok(outcome(true))
        })
        .expect("writer must survive a failed mutation");
...
fn journal_failure_rolls_back_the_mutation_and_releases_the_transaction() {
        ...
        .write(|c| {
            ...
            Ok(WriteOutcome {
                ...
                events: vec![PendingEvent {
                    entity_type: EntityType::Other("recovery".into()),
                    entity_id: "1".into(),
    ...
    store
        ...
        .expect("journal failure must release the transaction");
    assert_eq!(count(&store), 1);
}
...
fn commit_failure_rolls_back_and_the_next_write_succeeds() {
    ...
    store
        ...
        .expect("failed COMMIT must release the transaction");
    assert_eq!(count(&store), 1);
}
...
async fn health_refuses_a_store_that_can_read_but_cannot_write() {
    ...
    let state = amux_server::api::AppState {
        ...
        build_hash: "writer-recovery-test".into(),
        auth_token: None,
...
async fn stalled_writer_health_is_bounded_and_recovers_without_changing_revision() {
    ...
    assert!(
        ...
        "health must not wait behind a stuck writer"
    );
    for _ in 0..100 {
        let (code, _) = amux_server::api::health::health(axum::extract::State(state.clone())).await;
        if code.as_u16() == 200 {
            assert_eq!(
                ...
                initial,
                "health must not generate sync revisions"
            );
            return;
        }
