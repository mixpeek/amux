...
async fn call(
    ...
    if !worker.is_empty() {
        req = req.header("X-Amux-Worker", worker);
    }
...
async fn workers_keep_assignments_and_dependencies_on_their_own_board() {
    ...
    for lane in ["owner", "peer"] {
        std::fs::write(
            home.path().join(format!("sessions/{lane}.env")),
            "CC_DIR=/tmp\n",
        )
        .unwrap();
    }
    ...
    for route in [json!({"request_to":"peer"}), json!({"session":"peer"})] {
        ...
        b.as_object_mut()
            .unwrap()
            .extend(route.as_object().unwrap().clone());
        let (s, v) = call(&app, "POST", "/api/board", "owner", b).await;
        assert!(
            matches!(s, StatusCode::CONFLICT | StatusCode::FORBIDDEN),
            "{v}"
        );
    }
    let mut b = create_body("Not a cross-board wait");
    b["depends_on"] = json!([peer_id]);
    let (s, v) = call(&app, "POST", "/api/board", "owner", b).await;
    ...
    assert!(v["how_to_fix"]
        .as_str()
        .unwrap()
        .contains("Do not relocate a peer wait"));
    ...
    let (s, v) = call(
        ...
        &format!("/api/board/{id}"),
        "owner",
        json!({"depends_on":[peer_id],"desc_append":"must not persist"}),
    )
    .await;
    assert_eq!(s, StatusCode::CONFLICT, "{v}");
    assert_eq!(v["code"], "cross_board_dependency_forbidden");
    ...
    let (s, v) = call(
        ...
        &format!("/api/board/{id}"),
        "owner",
        json!({"depends_on":[local_id]}),
    )
    .await;
    assert_eq!(s, StatusCode::OK, "{v}");
    let (_, before_move) = call(
        &app,
        "GET",
        &format!("/api/board/{local_id}"),
        "owner",
        Value::Null,
    )
    .await;
    let (s, v) = call(
        ...
        &format!("/api/board/{local_id}"),
        "",
        json!({"session":"peer", "desc_append":"must not persist"}),
    )
    .await;
    assert_eq!(s, StatusCode::CONFLICT, "{v}");
    assert_eq!(v["relation"], "incoming_dependents");
    let (_, after_move) = call(
        &app,
        "GET",
        &format!("/api/board/{local_id}"),
        "owner",
    ...
    store
        .write(move |conn| {
            conn.execute(
                "UPDATE issues SET status='verified', depends_on=?1 WHERE id=?2",
                rusqlite::params![json!([legacy_peer]).to_string(), legacy_id],
            )?;
    ...
    let (s, v) = call(
        ...
        &format!("/api/board/{id}"),
        "owner",
        json!({"status":"backlog", "depends_on":[local_id]}),
    )
    .await;
    assert_eq!(s, StatusCode::OK, "{v}");
    ...
    store
        .write(move |conn| {
            ...
            let new = bs::NewIssue {
                title: "Internal foreign dependency".into(),
                desc: String::new(),
                status: "backlog".into(),
            ...
            conn.execute(
                ...
                rusqlite::params![json!([peer_id]).to_string(), id],
            )
            .unwrap();
            ...
            conn.execute("UPDATE issues SET status='verified' WHERE id=?1", [id])
                .unwrap();
            legacy.status = "todo".into();
            assert!(
                bs::save_patched(conn, &mut legacy).is_err(),
                "reopening cannot reactivate a historical cross-board wait"
            );
            ...
            conn.execute("UPDATE issues SET session=NULL WHERE id=?1", [peer_id])
                .unwrap();
            assert_eq!(
                bs::foreign_dependencies(
                    conn,
                    &bs::BoardOwner::new(None, Some("owner")),
                    &[peer_id.into()]
                )
                .unwrap()
                .len(),
            ...
            assert!(bs::foreign_dependencies(
                conn,
                &bs::BoardOwner::new(None, None),
                &[peer_id.into()]
            )
